...
export default function AuditView() {
  ...
  const fetchAudits = async () => {
    const { data } = await fetch('/api/map').then(res => res.json());
  ...
  const runAgenticAudit = async () => {
    ...
    setIsAnalyzing(true);
    setAnalysisStep("Uploading Evidence...");

    try {
      // 1. Get Geolocation
      setAnalysisStep("Acquiring GPS Coordinates...");
      ...
      formData.append('notes', 'Citizen report submitted via Audit View');
      ...
      setAnalysisStep("Running AI Audit...");
      ...
      if (data.success && data.audit) {
        setAudit({
          riskLevel: data.audit.risk_level,
          discrepancies: data.audit.discrepancies,
          reasoning: data.audit.ai_verdict,
          confidenceScore: 0.95, // Default for now
        });
        // Refresh the log
        fetchAudits();
      } else {
      ...
      alert("Failed to complete audit. Please check your connection and try again.");
    } finally {
...
**Subject:** Formal Complaint regarding Project "${MOCK_RECORD.projectName
      }" - Audit Non-Compliance

Respected Sir/Madam,

I am writing to formally report serious irregularities identified in the "${MOCK_RECORD.projectName
...
Yours faithfully,`;
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 space-y-6 font-sans text-slate-900">
      ...
      <motion.div
        layout
        className={`bg-white rounded-xl border-l-4 shadow-md overflow-hidden transition-colors duration-500 ${isHighRisk
      ...
      >
        <div className="p-6">
          <div className="flex justify-between items-start mb-6">
            ...
            <div
              className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border ${isHighRisk
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        <div
          ...
          className={`relative group border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer overflow-hidden ${selectedImage
        ...
        <Button
          ...
          className={`w-full h-12 text-base font-semibold transition-all ${isAnalyzing
      ...
      {/* RECENT AUDITS LOG */}
      <div className="mt-12">
        <h3 className="text-lg font-bold text-slate-900 mb-4">Recent Audit Log</h3>
        <div className="space-y-4">
          {recentAudits.map((report) => (
            <div key={report.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex justify-between items-center">
              <div>
                <h4 className="font-bold text-slate-800 text-sm">
                  {report.audit_results?.[0]?.government_projects?.project_name || "Unknown Project"}
                </h4>
                <p className="text-xs text-slate-500 mt-1">
                  {new Date(report.timestamp).toLocaleDateString()} • ID: {report.id.slice(0, 8)}
                </p>
              </div>
              <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${report.audit_results?.[0]?.risk_level === 'High' ? 'bg-red-50 text-red-600' :
                report.audit_results?.[0]?.risk_level === 'Medium' ? 'bg-amber-50 text-amber-600' :
                  'bg-green-50 text-green-600'
                }`}>
                {report.audit_results?.[0]?.risk_level || "Pending"}
              </div>
            </div>
          ))}
          {recentAudits.length === 0 && (
            <p className="text-slate-500 text-sm italic">No recent audits found.</p>
          )}
        </div>
      </div>
    </div>
