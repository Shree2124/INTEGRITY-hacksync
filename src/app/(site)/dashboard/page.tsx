...
const MY_REPORTS = [
  { id: 'REP-8821', project: 'Dadar Station Skywalk', date: '2 Oct 2025', status: 'Verified', risk: 'High', verdict: 'Structural cracks detected.' },
  { id: 'REP-9921', project: 'Linking Road Potholes', date: '15 Sep 2025', status: 'Resolved', risk: 'Medium', verdict: 'Uneven surface.' }
];

const MY_DOCS = [
  { id: 'RTI-2025-001', title: 'Tender Docs - Dadar Skywalk', date: '3 Oct 2025', status: 'Ready', type: 'RTI Request' },
  { id: 'CMP-2025-042', title: 'Linking Road Negligence', date: '16 Sep 2025', status: 'Submitted', type: 'Formal Complaint' }
];
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          ...
          const def = { lat: 19.0760, lng: 72.8777 };
          setUserLocation(def);
        }
      );
    }
    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      `}>
        <div className=" flex justify-between items-center">
          ...
          <button onClick={() => setIsMobileNavOpen(false)} className="md:hidden text-slate-400">
            <X size={24} />
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full relative">
        ...
        <div className="md:hidden bg-white border-b border-slate-200 p-4 flex items-center justify-between shrink-0 relative z-[1500]">
           ...
           <button onClick={() => setIsMobileNavOpen(true)} className="p-2 bg-slate-100 rounded-lg">
             <Menu size={20} />
           </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6 scroll-smooth relative z-0">
          <div className="max-w-7xl mx-auto">
            ...
            {activeTab === 'overview' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                   <div>
                      <h2 className="text-xl font-bold text-slate-900">City Pulse</h2>
                      <p className="text-slate-500 text-xs">Monitoring infrastructure health in real-time.</p>
                   </div>
                   <div className="flex gap-2">
                      <div className="px-3 py-1.5 bg-white rounded-lg border border-slate-200 shadow-sm text-xs font-bold text-slate-700 flex items-center gap-2">
                         ...
                         {userLocation ? "Vasai-Virar (Active)" : "Locating..."}
                      </div>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                   ...
                   <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[350px]">
                      <div className="p-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
                         <h3 className="font-bold text-slate-800 text-sm">Nearby Projects</h3>
                         <span className="text-[10px] bg-slate-200 px-2 py-0.5 rounded-full text-slate-600 font-medium">2 Active</span>
                      </div>
                      <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                         {MOCK_RECORDS.map(record => (
                            <div 
                              ...
                              className={`p-3 rounded-lg border cursor-pointer transition-all hover:shadow-md ${selectedRecord?.id === record.id ? 'bg-slate-900 text-white border-slate-900' : 'bg-white border-slate-100 hover:border-blue-300'}`}
                            >
                               <div className="flex justify-between items-start mb-1">
                                  <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${selectedRecord?.id === record.id ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-500'}`}>{record.id}</span>
                                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${record.status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                                    {record.status}
                                  </span>
                               </div>
                               <h4 className="font-bold text-xs leading-tight mb-1">{record.projectName}</h4>
                               <div className={`text-[10px] ${selectedRecord?.id === record.id ? 'text-slate-400' : 'text-slate-500'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-2">
                   <div className="p-4 bg-blue-600 rounded-xl text-white shadow-lg shadow-blue-200 flex justify-between items-center">
                      <div>
                         <p className="text-blue-100 text-[10px] font-bold uppercase">My Contributions</p>
                   ...
                   <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm flex justify-between items-center">
                      <div>
                         <p className="text-slate-500 text-[10px] font-bold uppercase">Alerts Near Me</p>
                   ...
                   <Link href="/createaudit" className="w-full h-full">
                     <button 
                        className="group h-full p-4 bg-slate-900 rounded-xl text-white shadow-sm flex items-center justify-between cursor-pointer hover:bg-slate-800 transition-colors w-full text-left"
                     >
                        <div className="flex items-center gap-3">
                           <div className="p-2 bg-slate-700 rounded-lg"><Camera size={18} /></div>
                           <div className="flex flex-col">
                              <span className="font-bold text-sm">Create Audit Draft</span>
                              <span className="text-[10px] text-slate-400">Report an issue</span>
                           </div>
                        </div>
                       ...
                       <tbody className="divide-y divide-slate-100">
                          {MY_REPORTS.map((report) => (
                             <tr key={report.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4 font-bold text-slate-900">{report.project}</td>
                                <td className="px-6 py-4"><span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold">{report.status}</span></td>
                                <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-bold ${report.risk === 'High' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{report.risk}</span></td>
            ...
            {activeTab === 'rti' && (
              <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                 <h2 className="text-2xl font-bold text-slate-900">RTI & Complaints</h2>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {MY_DOCS.map((doc) => (
                       <div key={doc.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:border-blue-400 transition-all group">
                          <div className="flex justify-between items-start mb-4">
                             <div className="p-3 bg-blue-50 text-blue-600 rounded-lg"><FileText size={24} /></div>
                             <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold">{doc.status}</span>
                          </div>
                          <h3 className="text-lg font-bold text-slate-900 mb-1">{doc.title}</h3>
                          <p className="text-xs text-slate-500 font-mono mb-4">{doc.id} • {doc.date}</p>
